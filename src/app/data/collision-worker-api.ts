...
import { rehydrateCollisionFamily, rehydrateSystemTree, findBodyInTree } from './collision-request';
...
export interface CollisionWorkerApi {
  ...
  /** `partnerName` is re-resolved against the rehydrated tree — see {@link findBodyInTree}. */
  ringContactsWithin(dto: SystemTreeDto, partnerName: string, horizonDays: number, now: number): CollisionWindow[];
  /** `partnerName` is re-resolved against the rehydrated tree — see {@link findBodyInTree}. */
  ringSeparationSeries(dto: SystemTreeDto, partnerName: string, startMs: number, endMs: number, samples: number): SeparationSample[];
}
...
export function createCollisionApi(core: OrbitalRelationsCore = new OrbitalRelationsCore()): CollisionWorkerApi {
  return {
    ...
    ringContactsWithin: (dto, partnerName, horizonDays, now) => {
      const self = rehydrateSystemTree(dto);
      const partner = findBodyInTree(self, partnerName);
      return partner ? core.ringContactsWithin(self, partner, horizonDays, now) : [];
    },
    ringSeparationSeries: (dto, partnerName, startMs, endMs, samples) => {
      const self = rehydrateSystemTree(dto);
      const partner = findBodyInTree(self, partnerName);
